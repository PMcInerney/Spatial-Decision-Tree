    ...
    for edge in edges:
        ...
        while not (cellx == end_cellx and celly == end_celly):
            ...
            if slope_Is_Positive:
                ...
                if (point2[0]-point1[0])*((celly+1)*64-point1[1])-(point2[1]-point1[1])*((cellx+1)*64 - point1[0]) >= 0:
                ...
                ##(Bx - Ax) * (Cy - Ay) - (By - Ay) * (Cx - Ax)
                if (point2[0]-point1[0])*((celly)*64-point1[1])-(point2[1]-point1[1])*((cellx+1)*64 - point1[0]) <= 0 :
...
def generate_event_im_pairs(events,waves,im_files):
    # takes a data structure listing HEK events and a folder of images and 
    # associates each event with the closest image in the folder
    ...
    read_data = events
    headers = read_data[0]
    allEvents = read_data[1:]   # the first row is column headers
    if not len(waves) ==0:
      eventsWeWant = [event for event in allEvents if int(pt.parseWave(event,headers)) in waves ]
    else:
      eventsWeWant = allEvents
    ...
    event_im_pairs = []
#    event_im_pairs = [[event,findClosestImage(event,headers,ims)] for event in eventsWeWant]
    count = 0
    for event in eventsWeWant:
      count += 1
      print count,'/',len(eventsWeWant)
      x = findClosestImage(event,headers,ims)
      event_im_pairs.append([event, x])
    return event_im_pairs

def findClosestImage(event,headers,ims):
    ...
    if len(ims)==1:
        ...
        if abs (ims[index][1] - pt.parseMidTime(event,headers)) < abs(ims[index-1][1]-pt.parseMidTime(event,headers)):
            return findClosestImage(event,headers,ims[index:])
        else:
            return findClosestImage(event,headers,ims[:index])
